...
export default class Resource extends TerraformResourceCommand {
  static description = 'Manage Spot instance'
  ...
  getProductByState(state: any) {
    const data = get(state, 'product')
  ...
  async create(data: any): Promise<any> {
    ...
    return this.createStateByOrderId(data, createOrderResponse.data.id)
  }
  ...
  async read(state: any): Promise<any> {
    ...
    // retrieve product
    const product = this.getProductByState(state)
    const externalId = product.externalId
  ...
  async update(state: any, data: any): Promise<any> {
    ...
    // create new state
    const vm = await this.getVmDetails(product.externalId)

    // expired resource
    return vm.status === 'DELETED' ? this.create(data) : this.createStateByOrderId(data, orderId)
  }
  ...
  async delete(state: any): Promise<any> {
    ...
    return this.getOrderDetails(response.data.id)
  }
}
